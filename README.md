...
- When refreshing the browser, the game checks for the past contract events, from 1000000 blocks before the current one, this can be slow and might not work if there are older events. Also, sometimes, the event is not triggerred or it takes too much time to be triggerred. Solutin to both issues: Use an indexer such as Moralis or The Graph
